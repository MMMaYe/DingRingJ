 ...
 * <p>场景配置：
 * <ul>
 *   <li>讨论场景（CHAT/DISCUSS/CONCLUDE）：recursionLimit=3，不深陷工具循环</li>
...
public class SaaReactAgentFactory {
    ...
    /**
     * 构建讨论场景 ReactAgent（recursionLimit=3，轻量工具）。
     *
    ...
    public ReactAgent buildDiscussAgent(Agent domainAgent, String systemPrompt, ToolSet toolSet) {
        return build(domainAgent, systemPrompt, toolSet, 3);
    }

    /**
     * 构建工作场景 ReactAgent（recursionLimit=15，通用工具集）。
     */
    public ReactAgent buildWorkAgent(Agent domainAgent, String systemPrompt) {
        return build(domainAgent, systemPrompt, ToolSet.WORK, 15);
    }
