...
class InjectKbHookTest {
    ...
    void shouldInjectBothSourcesOnDiscuss() {
        when(ragService.retrieve("如何学好 JMM", 1L)).thenReturn("知识库检索结果（按相关性排序）：\n[1] (相关性: 8.0) JMM 三大特性");
    ...
    void shouldInjectOnlyKbSourceOnWork() {
        when(ragService.retrieve("帮我整理部署文档", 1L)).thenReturn("知识库检索结果（按相关性排序）：\n[1] 部署手册");
    ...
    void shouldFallBackToKbOnlyWhenIntentMissing() {
        when(ragService.retrieve(anyString(), anyLong())).thenReturn("知识库检索结果（按相关性排序）：\n[1] x");
